...
api_v1_urlpatterns = [
    path(
        ...
        include(("deals.apps.accounts.api.v1.urls", "accounts"), namespace="api-v1-accounts"),
    ),
    path(
            f"{API_PREFIX}/v1/",
            include(("deals.apps.processor.api.v1.urls", "processor"), namespace="api-v1-processor"),
        )
    ]
...
# enable Swagger
if "SWAGGER" in settings.DEALS_FEATURES:
    ...
    api_v1_schema_view = get_schema_view(
        openapi.Info(
            title="deals",
            default_version="v1",
            description="deals API v1 description",
        ),
        public=True,
