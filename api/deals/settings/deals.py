...
DEALS_FEATURES = env.list(
    "CORE_FEATURES",
    default=[],
)

# Reset password link lifetime interval (in seconds). By default: 1 hour.
DEALS_RESET_PASSWORD_EXPIRATION_DELTA = timedelta(
    seconds=env.int(
        "CORE_RESET_PASSWORD_EXPIRATION_DELTA",
        default=3600,
    ),
)

if "LOG_SQL" in DEALS_FEATURES:  # pragma: no cover
    LOGGING = {
        "version": 1,
