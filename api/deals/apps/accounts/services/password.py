...
from deals.apps.accounts.exceptions import (
...
class PasswordService:
    ...
    @classmethod
    def send_reset_password_link(cls, username: str) -> None:
        try:
            user = UserAccount.objects.active().get(username=username)
    ...
    def reset_password(cls, signature: str, new_password: str) -> None:
        ...
        try:
            user_pk = signer.unsign(signature, max_age=settings.DEALS_RESET_PASSWORD_EXPIRATION_DELTA)
