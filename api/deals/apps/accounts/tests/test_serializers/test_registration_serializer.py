...
from deals.apps.accounts.api.v1.serializers.registration import RegistrationSerializer
...
def test_registration_serializer_validate_username_success(user_account, input_data):
    user_account(username="john")
    serializer = RegistrationSerializer(data=input_data)
    validated_username = serializer.validate_username(input_data["username"])
    assert validated_username == input_data["username"]


@pytest.mark.django_db
def test_registration_serializer_validate_username_failure(user_account, input_data):
    user_account(username=input_data["username"])
    serializer = RegistrationSerializer(data=input_data)
    with pytest.raises(ValidationError):
        serializer.validate_username(input_data["username"])
