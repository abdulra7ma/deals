...
def test_create_user_success():
    ...
    user = UserAccount.objects.create_user(user_username, user_password)
...
@pytest.mark.django_db
def test_create_user_no_username_failure():
    assert UserAccount.objects.count() == 0
    ...
    assert UserAccount.objects.count() == 0
    assert "Users must give an username address" in str(exc_info.value)
...
def test_create_superuser_success():
    ...
    superuser_username = "admin@example.com"
    superuser_password = "super-secret-password"  # nosec
    superuser = UserAccount.objects.create_superuser(superuser_username, superuser_password)

    assert UserAccount.objects.count() == 1
    assert UserAccount.objects.first() == superuser
    assert superuser.username == superuser_username
...
@pytest.mark.django_db
def test_create_superuser_no_username_failure():
    assert UserAccount.objects.count() == 0
...
@pytest.mark.parametrize("username", ["jane", "john"])
@pytest.mark.django_db
def test_get_short_name(user_account, username):
    assert UserAccount.objects.count() == 0
...
@pytest.mark.django_db
def test_get_full_name(user_account, first_name, last_name, username, expected):
    assert UserAccount.objects.count() == 0

    user = user_account(first_name=first_name, last_name=last_name, username=username)
