...
LOGIN_SERIALIZER_PATH = "deals.apps.accounts.api.v1.serializers.login.LoginSerializer"
...
def test_login_api_success(user_account, unauthorized_api_client, mocker):
    ...
    mocked_login = mocker.patch("deals.apps.accounts.services.login.LoginService.login", return_value=mocked_response)

    data = {"username": "jane@example.com", "password": "super-secret-password"}  # nosec
...
def test_login_api_wrong_credentials_failure(user_account, unauthorized_api_client, mocker):
    mocked_validate = mocker.patch(
        f"{LOGIN_SERIALIZER_PATH}.validate", side_effect=[ValidationError(gettext("Incorrect username or password."))]
    )
    mocked_login = mocker.patch("django.contrib.auth.login")

    data = {"username": "jane", "password": "super-secret-password"}  # nosec
    response = unauthorized_api_client.post(reverse("api-v1-accounts:login"), data)

    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.data == {"non_field_errors": [gettext("Incorrect username or password.")]}
    mocked_validate.assert_called_once_with(data)
